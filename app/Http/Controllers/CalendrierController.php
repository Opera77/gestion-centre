...
class CalendrierController extends Controller
{
    public function export(Request $request)
    {
        ...
        // Filtrer les activités contenant "OpenLab" ou "open lab" sans tenir compte de la casse
        $openLabActivities = $mergedActivites->filter(function ($activity) {
            return stripos($activity->title, 'OpenLab') !== false || stripos($activity->title, 'open lab') !== false;
        });

        // Définir la cellule de départ pour l'affichage des titres
        $startColumn = 'D';
        $startRow = 8;  // Point de départ initial

        // Placer les titres des activités dans la feuille de calcul à partir de la cellule D8
        foreach ($openLabActivities as $activity) {
            // Extraire la date de l'activité et formater la date sans l'année
            $activityDate = new DateTime($activity->start_date);
            $formattedDate = $activityDate->format('d-m');
            
            // Créer le titre à afficher (par exemple, "OpenLab du 12-02")
            $titleToDisplay = "OpenLab du " . $formattedDate;
            
            // Calculer la position de la cellule courante
            $currentCell = $startColumn . $startRow;
            
            // Écrire le titre formaté dans la cellule courante
            $sheet->setCellValue($currentCell, $titleToDisplay);
            
            // Appliquer les styles à la cellule courante
            $sheet->getStyle($currentCell)
                ->getFont()
                ->setName('Arial')  // Police Arial
                ->setSize(10)       // Taille 10
                ->setBold(false);    // Texte en gras
            $sheet->getStyle($currentCell)
                ->getAlignment()
            ...
            // Incrémenter $startRow pour la prochaine activité
            $startRow++;
        }
        ...
        // Définir la valeur à écrire dans la cellule A7
        $semestreText = ($semestre == 1) ? 'S1' : 'S2';
        $textToDisplay = "$semestreText $annee";

        // Écrire la valeur dans la cellule A7
        $sheet->setCellValue('A7', $textToDisplay);
        
        // Appliquer les styles (gras, couleur noir, taille 18) à la cellule A7
        $sheet->getStyle('A7')->getFont()->setBold(true)->setSize(18)->getColor()->setARGB('FF000000');
        ...
        // Sélection des mois en fonction du semestre
        if ($semestre == 1) {
        ...
        $daysOfWeek = [1 => 'Lundi', 2 => 'Mardi', 3 => 'Mercredi', 4 => 'Jeudi', 5 => 'Vendredi', 6 => 'Samedi']; // Exclusion de dimanche
        ...
        $sheet = $spreadsheet->getActiveSheet();
        $currentYear = $annee; // Utilisation de l'année sélectionnée

        foreach ($months as $index => $month) {
            $startColumnIndex = $index * 12 + 1;
            $currentColumn = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($startColumnIndex);
            $date = new DateTime("{$currentYear}-" . (($semestre == 1 ? $index + 1 : $index + 7)) . "-01");
            $daysInMonth = $date->format('t');
            $lastDayRow = $startRow + 1 + $daysInMonth;

            for ($day = 1; $day <= $daysInMonth; $day++) {
                $date->setDate($currentYear, ($semestre == 1 ? $index + 1 : $index + 7), $day);
                $dayOfWeekNumber = $date->format('w');
                
                // Si c'est un dimanche (w = 0), on saute
                ...
                foreach ($filteredActivities as $activity) {
                    ...
                    if ($activity->categorie_id == 2) {
                        ...
                        $activityColumn = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($startColumnIndex + 6);
                    }
                    ...
                    if ($duration <= 3) {
                        ...
                        $endCellCoordinate = $startCellCoordinate;
                    }
